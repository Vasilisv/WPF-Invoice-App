...
namespace WpfApp1
{
    ...
    public partial class InvoiceWindow : Window
    {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            dlg.FilterIndex = 0;
            dlg.FileName = "ΤΠ-" + invoiceid.Text;
        ...
        public void GetInvoiceID()
        {
            ...
            command.Parameters.AddWithValue("@userid", LoginWindow.id);
            command.CommandText = "SELECT * FROM SavedInvoices WHERE userid=@userid ORDER BY id DESC LIMIT 1 ";
            MySqlDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                int x = reader.GetInt32("InvoiceId") + 1;
            ...
            {
                MessageBox.Show("Please enter a starting Invoice ID!");
            }
    ...
    public class MainViewModel
    {
        ...
        public MainViewModel()
        {
            ...
            MySqlCommand command = cnn.CreateCommand();
            command.Parameters.AddWithValue("@id", LoginWindow.id);
            command.CommandText = "SELECT name FROM clients WHERE userid=@id";
            MySqlDataReader reader = command.ExecuteReader();
            ...
            connectionstring2 = myConnection.connectionString();
            MySqlConnection cnn2 = new MySqlConnection(connectionstring2);
            cnn2.Open();
            MySqlCommand command2 = cnn2.CreateCommand();
            command2.Parameters.AddWithValue("@id", LoginWindow.id);
            command2.CommandText = "SELECT name,dimX,dimY FROM products WHERE userid=@id";
            MySqlDataReader reader2 = command2.ExecuteReader();
            ...
            if (reader2.HasRows)
            {
                while (reader2.Read())
                {
                    name2 = reader2.GetString("name");
                    dimX = reader2.GetFloat("dimX");
                    dimY = reader2.GetFloat("dimY");

                    Products.Add(new Product { Name = String.Join("", name2, dimX, "x", dimY) });
                }
            }
